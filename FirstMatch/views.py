        ...
        if serializer.validated_data.get('Exclusionary_Criteria') == False:
            ...
            for col in cols:
                if col in dummies.columns:
                    ...
                    dummies[col] = 0
            numeric_cols = ['Gender', 'LS_Type', 'CYF_code', 'RefSourceName',
                            'EpisodeNumber',
            ...
            Feature_names = ['EpisodeNumber', 'Number of foster care placements', 'AgeAtEpisodeStart',
                             ...
                             'YLS_Leisure_Score', 'YLS_Personality_Score', 'YLS_Attitude_Score',
                             'Client self-harm',
            ...
            if query.count() > 0:
                if serializer.validated_data.get('gender') == 1:
                    ...
                    serializer.save(roc_confidence = roc_confidence, program=program_num[0], condition_program=condition_program,
                                    ...
                                    model_program=program_model_suggested_list[0])  # confidence = PC_proba[0][1]*100
                        ...
                        if program_pred == 2:
                            ...
                            if (drugUse == 0 and ylsSUBAB == 0 and alcholUSe == 0):
                                ...
                                for i in query_default:
                                    level_default.append(i.level_of_care)
                                    facility_default.append(i.facility_type)
                                confidence = program_condition(p13_model_preds, level_default[0], facility_default[0])[0]
                                roc_confidence = program_condition(p13_model_preds, level_default[0], facility_default[0])[1]
                                serializer.save(roc_confidence=roc_confidence,program=p13_model_preds, confidence=confidence,
                                                family_support=data['Family support'][0],
                                                level_of_aggression=data['Level of aggression'][0],
                                                fire_setting=data['Fire setting'][0],
                                                client_self_harm=data['Client self-harm'][0],
                            ...
                            for i in query_default:
                                level_default.append(i.level_of_care)
                                facility_default.append(i.facility_type)
                            confidence = program_condition(program_pred, level_default[0], facility_default[0])[0]
                            roc_confidence = program_condition(program_pred, level_default[0], facility_default[0])[1]
                            serializer.save(roc_confidence=roc_confidence,program=program_num[0], confidence=confidence,
                                            family_support=data['Family support'][0],
                                            level_of_aggression=data['Level of aggression'][0],
                                            fire_setting=data['Fire setting'][0],
                                            client_self_harm=data['Client self-harm'][0],
                ...
                if serializer.validated_data.get('gender') == 1:
                    ...
                    confidence = program_condition(condition_program, level_default[0], facility_default[0])[0]
                    roc_confidence = program_condition(condition_program, level_default[0], facility_default[0])[1]
                    serializer.save(roc_confidence=roc_confidence,program=program_num[0], condition_program=condition_program, confidence=confidence,
                                    family_support=data['Family support'][0],
                                    level_of_aggression=data['Level of aggression'][0],
                                    fire_setting=data['Fire setting'][0],
                                    client_self_harm=data['Client self-harm'][0],
...
class Adelphoi_location(ListAPIView):
    def get(self, request, *args, **kwargs):
        ...
        if mt.exists():
            ...
            query = Adelphoi_Mapping.objects.filter(
                program_model_suggested=referred_program)  # program_name=referred_program, gender=gender_list[0]
            suggested_location = []
...
class AdelphoiResult(UpdateAPIView):  # UpdateAPIView
    ...
    def put(self, request, *args, **kwargs):
        ...
        if mt:
            ...
            mt.save()
            logger.info(
                'AdelphoiResult---client_selected_level,client_selected_facility,client_selected_program & client_selected_locations saved')
...
class Location_Mapping(UpdateAPIView):
    ...
    def put(self, request, *args, **kwargs):
        ...
        if mt.exists():
            ...
            mt.save()
            logger.info('Location_Mapping---client_selected_locations are saving')
            return Response({"data": "success"})
        else:
...
def saveData(request):
    if request.method == 'POST':
        ...
        Adelphoi_Mapping(location=list(eval(location)), location_names=location_names.split(','), program=program,
...
def programSave(request):
    if request.method == 'POST':
        ...
        try:
            program_name = data['program_name']
            ProgramModel(program_name=program_name,
                         program=ProgramModel.objects.count() + 1).save()  # ,program = ProgramModel.objects.count() + 1
...
def loactionSave(request):
    if request.method == 'POST':
        ...
        try:
            ...
            ModelLocation(location_names=location_names,
                          location=ModelLocation.objects.count() + 1).save()  # ,program = ProgramModel.objects.count() + 1
            result = ModelLocation.objects.filter(location_names=location_names)[0]
...
class LocationModify(RetrieveUpdateAPIView):
    ...
    def put(self, request, *args, **kwargs):
        ...
        serializer = serializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        mt: ModelLocation = ModelLocation.objects.filter(location=kwargs['pk']).first()
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if gender == 1:
                ...
                gender_name = "Male"
            facility_query = FacilityModel.objects.filter(facility_type=facility_type)[0]
            facility_names = facility_query.facility_names
            level_of_query = LevelModel.objects.filter(level_of_care=level_of_care)[0]
            level_names = level_of_query.level_names

            Adelphoi_Mapping(program=program, program_name=program_name, gender=gender, location=location,
                             location_names=location_names, gender_name=gender_name, facility_names=facility_names,
                             level_names=level_names, facility_type=facility_type, level_of_care=level_of_care,
                             program_model_suggested=program_model_suggested, program_type=program_type).save()
        ...
        try:
            ...
            if condition_program > 3:
                ...
                dt = {'EpisodeNumber': mt.episode_number,
                      'Number of foster care placements': mt.number_of_foster_care_placements,
                      'AgeAtEpisodeStart': mt.ageAtEpisodeStart,
                      'Number of prior placements \n(excluding shelter and detention)': mt.number_of_prior_placements,
                      'Number of prior treatment terminations (excluding shelter or detention)': mt.number_of_prior_treatment_terminations,
                      'Length of time since living at home': mt.length_of_time_since_living_at_home,
                      'Termination directly to AV': mt.termination_directly_to_AV,
                      'Death Caregiver': mt.death_Caregiver, 'Borderline IQ (below 70)': mt.borderline_IQ,
                      ...
                      'Significant mental health symptoms': mt.significant_mental_health_symptoms,
...
def referralSave(request):
    if request.method == 'POST':
        ...
        try:
            ...
            print(referral_name)
            # print(ReferralSource.objects.filter(referral_name=referral_name))
            ReferralSource(referral_name=referral_name,referral_code=ReferralSource.objects.count() + 1,id =ReferralSource.objects.count() + 1 ).save()
            result = ReferralSource.objects.filter(referral_name=referral_name)[0]
            rf_code = result.referral_code
            logger.info('referralSave---referral saved based on id')
            return JsonResponse({ "referral_code": rf_code,"Sucess": "true"}) #"referral_code": rf_code,
        except:
...
class referralModify(RetrieveUpdateAPIView):
    ...
    def put(self, request, *args, **kwargs):
        ...
        mt: ReferralSource = ReferralSource.objects.filter(referral_code=kwargs['pk']).first()
        mt.referral_name = result
        mt.save()
        ReferralSource.objects.filter(referral_code=kwargs['pk']).update(referral_name=result)
        logger.info('referralModify--Referral name change')
        return Response({"referral_id": mt.referral_code, "Result": "Referral name is modified"})
