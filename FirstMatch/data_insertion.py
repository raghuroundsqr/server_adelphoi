...
from .serializers import (ModelTestsSerializers,
                          ProgramLocationSerialzer,
                          ProgramLevelSerialzer,
                          AdminInterface,
                          FilterSerialzer,
                          Adelphoi_placementSerializer,
                          Adelphoi_referredSerializer,
                          ProgramIndSerializer,
                          ProgramSerializer,
                          LocationSerializer,
                          LocationIndSerializer,
                          Available_programSerializer,
                          Program_PCRSerializer,
                          ReferralIndSerializer,
                          refferalSerializer)
...
def index(request, *args, **kwargs):
    template = loader.get_template(
        '/home/ubuntu/Adelphoi/adelphoi-django/templates/index2.html'
    ...
    if results.Exclusionary_Criteria:
        exc_crt = "Reject"
    else:
        exc_crt = "Accept"
    ...
    pdf = weasyprint.HTML(
        ...
        stylesheets=[
            CSS(
                '/home/ubuntu/Adelphoi/adelphoi-django/templates/index.css'
            )
        ],
        ...
        if not serializer.validated_data.get('Exclusionary_Criteria'):
            ...
            dt = {
                ...
                'Level of aggression': serializer.validated_data.get(
                    'level_of_aggression'),
                'Fire setting': serializer.validated_data.get(
                    'fire_setting'),
            ...
            Feature_names = [
                ...
                'Animal cruelty', 'Number of prior hospitalizations',
                'Compliant with medication',
                'Significant mental health symptoms',
                'Severe mental health symptoms',
                ...
                'Level of aggression', 'Fire setting', 'Abuse, or neglect',
                'Screening tool for Trauma--Total score'
            ]
            ...
            def program_condition(condition_program,
                ...
                Xp = pd.DataFrame(
                    data[[
                        ...
                        'Alcohol Use', 'Drug Use', 'Incarcerated caregivers',
                        'Incarcerated siblings', 'Number of prior AWOLS',
                        'Animal cruelty', 'Number of prior hospitalizations',
                        'Compliant with medication',
                        'Significant mental health symptoms',
                    ...
                    if serializer.validated_data.get('inclusionary_criteria'):
                        ...
                        if program_pred == 2:
                            ...
                            p13_model = pickle.load(
                                open("/home/ubuntu/Adelphoi/adelphoi-django/"
                                     "sources/R_LR_P13_28feb.sav",
                                     "rb"
                                     )
                            )
                            ...
                            serializer.save(
                                roc_confidence=roc_confidence,
                                program=program_num[0], confidence=confidence,
                                family_support=data['Family support'][0],
                                level_of_aggression=data[
                                    'Level of aggression'][0],
                                fire_setting=data['Fire setting'][0],
            ...
            serializer.save()
            return Response({
                "Result": "Thanx for registering with ADELPHOI"
            })
...
class Adelphoi_location(ListAPIView):
    def get(self, request, *args, **kwargs):
        ...
        if mt.exists():
            ...
            query = Adelphoi_Mapping.objects.filter(
                program_model_suggested=referred_program
...
class AdelphoiResult(UpdateAPIView):  # UpdateAPIView
    ...
    def put(self, request, *args, **kwargs):
        ...
        if mt:
            ...
            selected_facility = []
            query_location = Adelphoi_Mapping.objects.filter(
                program_type=client_selected_program)
...
def saveData(request):
    if request.method == 'POST':
        ...
        Adelphoi_Mapping(
            ...
            level_of_care=level_of_care, level_names=level_names,
...
class ProgramModify(RetrieveUpdateAPIView):
    ...
    def put(self, request, *args, **kwargs):
        ...
        mt.save()
        Adelphoi_Mapping.objects.filter(
            program=kwargs['pk']
        ).update(program_name=result)
        logger.info('ProgramModify--Program name change')
        return Response({
            "program_id": mt.program,
